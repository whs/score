...
async function getPasswordV1(fileId: string, username: string, password: string): Promise<string> {
 ...
 */
export async function getFileNameV1(fileId: string, username: string, password: string): Promise<string> {
	return `${fileId}/u${username}_${await getPasswordV1(fileId, username, password)}.json`
}

export async function pbkdf2(key: string, salt: string, iterations=10_000, hash = 'SHA-256') {
	let encoder = new TextEncoder();
...
async function getPasswordV2(fileId: string, username: string, password: string): Promise<string> {
 ...
 */
export async function getFileNameV2(fileId: string, username: string, password: string): Promise<string> {
